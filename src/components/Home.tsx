  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-4 select-none">
      ...
      {user.role?.toUpperCase() === 'ADMIN' && (
        <div className="mb-6 bg-brand-primary/10 border-2 border-brand-primary rounded-3xl p-6 sm:p-8 relative overflow-hidden transition-all duration-300 hover:shadow-lg hover:-translate-y-1">
          ...
          <div className="relative z-10 flex flex-col sm:flex-row items-center justify-between gap-6">
            <div className="flex items-center gap-4">
              <div className="w-14 h-14 bg-brand-primary/20 rounded-2xl flex items-center justify-center shrink-0 border border-brand-primary/30">
                <svg className="w-7 h-7 text-brand-primary" fill="none" viewBox="0 0 24 24" strokeWidth="2" stroke="currentColor">
              ...
              <div className="text-start">
                <h3 className="text-xl sm:text-2xl font-black text-brand-primary tracking-tight font-sans">
                  {lang === 'ar' ? 'لوحة تحكم المسؤول' : 'Admin Control Panel'}
                </h3>
                <p className="text-sm sm:text-base text-brand-dark/70 font-semibold mt-1">
                  {lang === 'ar' ? 'إدارة الطلبات، الحلقات، إحصائيات المعلمات والطالبات.' : 'Manage requests, sessions, and overall system statistics.'}
                </p>
              </div>
            </div>
            <button 
              onClick={() => {
                // Dispatch event or call a method to navigate to control panel
                // Currently Home.tsx does not have a direct route prop!
                // But we can trigger a custom event or redirect via window:
                const navBtn = document.querySelector('button[aria-label="controlpanel"]') as HTMLButtonElement;
                if(navBtn) navBtn.click();
                else window.dispatchEvent(new CustomEvent('nav-request', { detail: 'controlpanel' }));
              }}
              className="w-full sm:w-auto px-6 py-3.5 bg-brand-primary text-white rounded-xl font-black shadow-md hover:-translate-y-0.5 hover:shadow-lg transition-all"
            >
              {lang === 'ar' ? 'الدخول للوحة' : 'Open Panel'}
            </button>
          </div>
        </div>
      )}
